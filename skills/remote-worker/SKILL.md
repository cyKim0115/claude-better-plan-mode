...
4. **지시문 작성** — `job_submit`의 `prompt`는 저쪽 세션이 받는 전부다. 아래 "지시문" 절을 따른다.
   ...
   결과를 **화면으로 봐야 하는 작업**(UI 배치, 연출, 애니메이션 등)이면 `capture: true`를 붙인다.
   `worker_projects`가 그 프로젝트를 "화면 캡처: 가능"으로 표시할 때만 받는다. 코드만 고치는 작업에는 붙이지 않는다
   — 저쪽 PC에 에디터 창이 뜨고 시간이 더 걸린다.
...
7. **완료 처리** — `succeeded`면 PR 링크(또는 direct면 "기본 브랜치 반영됨, git pull 필요")를 전한다.
   ...
   `job_status`에 "화면 캡처: N개"가 있으면 보드 잡 상세에서 볼 수 있다고 안내한다. "없음"으로 나오면
   그 사유도 함께 전한다 — 잡 자체는 성공했어도 화면은 안 남았다는 뜻이다.
...
- **저쪽 PC가 지금 어떤 상태인지** — `worker_screenshot`. "지금 화면 어때", "Unity 상태 봐줘"가 여기다.
  이미지가 돌아오면 보이는 대로 설명한다 (에디터가 열려 있는지, 에러 창이 떠 있는지, 어떤 씬인지).
  잡이 `claude` 단계에서 오래 멈춰 있을 때 원인을 볼 용도로도 쓴다.
- **잡의 작업 결과가 어떻게 보이는지** — 제출할 때 `capture: true`. 저쪽 에디터가 게임을 실제로 띄워
  스크린샷·녹화를 남기므로, 사용자의 다른 창이 섞이지 않고 창이 가려져도 결과가 같다.
  이미 끝난 잡에는 소급 적용할 수 없다 — 화면이 필요하면 제출 시점에 켠다.
