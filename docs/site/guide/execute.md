...
PR 모드였다면 실행 패널과 진행 현황 패널에 **PR 링크**가 뜹니다. 커밋까지 갔지만 push·PR에서 실패한 경우 `/jobs/<id>` 상세에서 **커밋부터 마무리**로 그 단계부터 다시 돌릴 수 있습니다. 결과를 더 다듬어야 하면 같은 화면의 **이어서하기**로 그 세션을 그대로 재개해 추가 지시를 줄 수 있습니다 ([원격 워커](../integration/worker.md) 참고).
{% endstep %}
{% endstepper %}
