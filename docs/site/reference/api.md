...
| `POST /api/jobs` | `{ project, prompt, title?, mode?, model?, effort?, maxTurns?, skipPermissions?, parentJobId?, startFrom? }` | `{ id, status, branch }`. `parentJobId`를 주면 그 잡의 지시·세션 요약이 컨텍스트로 붙고, `startFrom: "parent"`면 그 잡 브랜치 위에서 worktree를 시작합니다 (기본 `"base"`) |
